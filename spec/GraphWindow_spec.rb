...
before(:all) do
  ...
  describe 'read_settings' do    
    ...
    it 'should return a hash with precisely 22 pairs' do
      expect(@default_settings.length).to eq(22)
    end
    ...
    it 'should return a hash with only simbols as keys' do
      @default_settings.each_key { |key| expect(key).to be_a(Symbol)}
    end
  end 



  describe 'take_and_process' do
    it 'shold have instanse variable with hash with 32 pairs' do
  ...
  describe 'rate_history' do
    ...
    it 'should should return a hash with only hashes in values ' do
      @history.each_value { |v| expect(v).to be_a(Hash)}
    end

    it 'should should return a hash with values sizes precisely 4' do
      @history.each_value { |v| expect(v.size).to eq 4}
    end

    it 'should return a hash with only floats in subhashes values' do
      @history.each_key do |k| 
        @history[k].each_value { |v| expect(v).to be_a(Float) }
      end
    end

    it 'returns a hash with only values greater than 0 in subhashes' do
      @history.each_key do |k| 
        @history[k].each_value { |v| expect(v).to be > 0}
      end
    end

    it 'returns a hash with no number bigger than max in subhashes values' do
      @history.each_key do |k| 
        @history[k].each_value { |v| expect(v).to be <= @history[k]['max'] }
      end
    end

    it 'returns a hash with no number less than min in subhashes values' do
      @history.each_key do |k| 
        @history[k].each_value { |v| expect(v).to be >= @history[k]['min'] }
      end
    end

    it 'returns a hash with exactly "start" "finish" "min" "max" strings as' + 
       'keys in subhashes values' do
